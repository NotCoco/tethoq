...
const api: DesktopHarnessApi = Object.freeze({
  ...
  updateState: (): ReturnType<DesktopHarnessApi["updateState"]> => ipcRenderer.invoke(IPC_CHANNELS.updateGetState),
  updateAction: (action: DesktopUpdateAction): ReturnType<DesktopHarnessApi["updateAction"]> => ipcRenderer.invoke(IPC_CHANNELS.updateAction, action),
  onUpdateState: (listener: (state: DesktopUpdateState) => void): (() => void) => subscribe<DesktopUpdateState>(IPC_CHANNELS.updateState, listener),
