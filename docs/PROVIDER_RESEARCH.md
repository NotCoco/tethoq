...
| Gemini CLI / installed Antigravity | DIRECT BYOK ONLY | Tethoq does not reuse either product's OAuth, keyring, login, or subscription. Antigravity's terms explicitly treat third-party software accessing the service through Antigravity OAuth as a breach: https://antigravity.google/terms. Google separately documents Gemini's OpenAI-compatible API and an Apache-2.0 Antigravity SDK authenticated with a Gemini API key. Tethoq therefore exposes stable `gemini-3.6-flash` through the Direct API wallet using `GOOGLE_API_KEY` or `GEMINI_API_KEY`; an installed-harness entry remains excluded unless Google supplies written permission or a compatible public contract. |
